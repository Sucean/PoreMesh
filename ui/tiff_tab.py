...
from src.walking_cubes import WalkingCubes
...
def create_tiff_tab():
    ...
    with gr.Row():
        with gr.Column():
            ...
            padding_size = gr.Slider(0, 20, value=8, label="Padding Size")
            threshold = gr.Slider(0, 255, value=127, label="Marching Cubes Threshold")
            tolerance = gr.Number(value=0.001, label="Snapping Distance")
            use_parallel = gr.Checkbox(label='Multiprocessing', value = True)
            use_legacy = gr.Checkbox(label='Use VTK/Legacy (gives faulty STLs)', value = False)
    ...
    tiff_btn.click(
        ...
        inputs=[tiff_file, block_size, cap_size, padding_size, threshold, cap_axis, working_dir, tolerance, use_parallel, use_legacy],
        outputs=[tiff_output]
    )


def mesh_worker(args):
    threshold, block, block_id, padding_size, working_dir, tolerance, use_legacy = args
    ...
    mesh_generator = MeshGenerator(threshold, tolerance)
...
def process_tiff_file(tiff_file, block_size, cap_size, padding_size, threshold, cap_axis, working_dir, tolerance, use_parallel, use_legacy):
    ...
    args_list = [(threshold, block, i, padding_size, working_dir, tolerance, use_legacy) for i, block in enumerate(blocks)]
